    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="glass-card p-6">
          ...
          {myBookings.length === 0 ? (
            <p className="text-white/60 text-center py-8">No tienes reservas todavía. ¡Reserva tu primer turno!</p>
          ) : (
        ...
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/60">
              {bookingStep === 1 && 'Selecciona una fecha (máximo 1 semana - Domingos cerrado)'}
          ...
          {bookingStep === 1 && (
            <div className="py-4">
              <div className="flex items-center justify-between mb-6">
                <Button onClick={previousMonth} variant="ghost" size="sm" className="text-white hover:bg-white/10">
                  <ChevronLeft className="w-5 h-5" />
                </Button>
                <h3 className="font-barlow text-xl font-bold text-white">
                  {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
                </h3>
                <Button onClick={nextMonth} variant="ghost" size="sm" className="text-white hover:bg-white/10">
          ...
          {bookingStep === 2 && (
            <div className="py-4">
              ...
              <p className="text-white mb-4">
                Fecha: <span className="text-primary font-bold">{selectedDate?.toLocaleDateString('es-AR')}</span>
              </p>

              {timeSlots.length === 0 ? (
                <p className="text-white/60 text-center py-8">No hay horarios disponibles</p>
              ) : (
          ...
          {bookingStep === 3 && (
            <div className="py-4">
              ...
              <p className="text-white mb-4">
                Hora: <span className="text-primary font-bold">{selectedTime}</span>
              </p>

              {(() => {
                const timeSlot = getCurrentTimeSlot();
                if (!timeSlot || !timeSlot.activities || timeSlot.activities.length === 0) {
                  return <p className="text-white/60 text-center py-8">No hay actividades disponibles</p>;
                }
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <div>
                            ...
                            {!activity.available && (
                              <p className="text-xs text-red-400 mt-1">Sin cupos</p>
                            )}
                          </div>
                          <div className="text-right">
              ...
              {selectedActivity && (
                <div className="mt-6 flex space-x-3">
                  ...
                  <Button onClick={() => setShowBookingFlow(false)} variant="outline" className="flex-1 border-white/20">
                    Cancelar
                  </Button>
                </div>
              )}
            ...
            <div className="space-y-4 py-4">
              <div className="bg-yellow-500/10 border border-yellow-500/20 p-4 rounded">
                <p className="text-yellow-400 text-sm">
                  <strong>Política:</strong> Si cancelas con más de 6 horas de anticipación, 
                  se devolverán los créditos. De lo contrario, no habrá devolución.
                </p>
              </div>
              ...
              <div className="flex space-x-3">
                <Button onClick={handleCancelBooking} variant="destructive" className="flex-1">
                  Sí, cancelar
                </Button>
                <Button onClick={() => setShowCancelModal(false)} variant="outline" className="flex-1 border-white/20">
                  No, mantener
                </Button>
              </div>
            </div>
      ...
      <Dialog open={showCreditsModal} onOpenChange={setShowCreditsModal}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/60">
              Para agregar créditos, contacta con nosotros
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="bg-primary/10 border border-primary/30 p-4 rounded">
              <p className="text-white text-sm">
                Para acreditar créditos a tu cuenta, comunícate con nosotros por WhatsApp.
              </p>
            </div>
            
            <Button onClick={handleAddCredits} className="w-full bg-green-600 hover:bg-green-700 text-lg py-6">
              <Phone className="w-5 h-5 mr-2" />
              Contactar por WhatsApp
            </Button>
            
            <Button onClick={() => setShowCreditsModal(false)} variant="outline" className="w-full border-white/20">
              Cerrar
            </Button>
          </div>
        </DialogContent>
